...
**Postcard（明信片）**:
对**一条或一组** Travel records 的**本地只读呈现**：选一款模板，配该次旅程的 Travel image、写寄语，盖上按旅程年份自绘的生肖纪念邮票与邮戳，导出成一张图片（1181×1748）。**不落库、不公开**——它是客户端的产物，不进入分享快照（ADR-0012 的"不含图片"边界不变），也不是可编辑的业务实体。
_Avoid_: 把明信片当记录/相册实体、把明信片当分享快照（快照是文字清单 + 链接）、把导出图片当作可再编辑的存档（明信片是一次性的，重开会重置）

**Postcard template / Postcard module（明信片模板 / 模块）**:
模板＝一套固定版式，并声明**要用几张图片、足迹清单最多几行、支持哪些模块**；模块＝可开关的画面元素（足迹清单 / 旅行标识 / 坐标小字 / 邮票 / 邮戳 / 收件人栏 / 地点日期）。用户只能在模板支持范围内开关模块；换模板保留内容与用户改过的开关。模板是前端只读清单（`postcard/postcard-template.ts`），不是用户数据。
_Avoid_: 把模板当可配置数据表、把模块当独立功能开关（它们只在明信片里生效）

**Stamp / Postmark（邮票 / 邮戳）**:
明信片上的两个**呈现元素**（不是业务实体）：邮票＝按旅程年份自绘的生肖纪念票（面值可改；真实邮票票面受版权保护，不内置，用户可上传自备图）；邮戳＝圆形日戳，地名取该记录的 City snapshot / 地点名快照，日期默认该次停留的到达日，压在邮票一角形成销票。
_Avoid_: 把邮票当可购买的资产、把邮戳地名当实时逆地理结果、把「旅行纪念」戳当成真实邮政业务戳

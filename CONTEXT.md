...
_Avoid_: 把不同城市的记录误并为一段；以为它与城市标记（地图聚合）粒度相同——City run 是「相邻同城」，城市标记是「全城一个」（见 ADR-0017）
...
**City marker（城市标记）**:
地图上一个城市的**聚合标记**：该城（Travel record 的 City 快照，或 Derived city）的全部停留合成一个标记，锚点在各次停留的中心；点它才展开该城的每一次停留（每行可定位、可详情、可再来）。与时光轴的 City run（相邻同城折叠）是**不同粒度**；既无快照又无派生的记录走地点标记（按近似坐标合并，ADR-0004）。
_Avoid_: 把城市标记当成城市的行政中心、以为它与时光轴节点一一对应

**Derived city（派生城市）**:
记录没有 City 快照时，在**读取期**按坐标向高德逆地理问来的城市短名；用于地图分组与图标派生，不写回既有记录。快照优先，派生只补空缺（ADR-0017）。例外给新数据：记录无快照时，「再来」会把派生城市带进**新记录**的预填，因此新记录会拿到 City 快照。
_Avoid_: 把派生城市当快照、把派生结果回写已有记录、要求它在时光轴/分享里也生效（目前只作用地图与侧栏行）

**Travel icon（旅行标识图标）**:
一条 Travel record 上内联的图标选择（Icon library 中的 key，如 `animal-panda`），用于在详情页、列表行、时光轴节点标识这次旅行。为空表示未显式选择——渲染期按 City snapshot 派生地区特色图标，该派生不落库（见 ADR-0016）。地图上城市标记用该城**首次到访**那条记录的解析结果（显式 `iconKey` > Regional icon）；地点标记要求组内解析结果一致才用图标，否则是默认标记加计数（见 ADR-0017）。

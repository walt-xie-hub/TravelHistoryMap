...
- 读取期派生不改写快照：历史数据里 City 为空的记录，地图会按坐标派生城市用于聚合展示（ADR-0017），
  但**不写回** `City`；需要补快照只能通过重新保存或将来单独的数据回填任务。
- API：DTO/请求带 `city`；无新端点。
- 前端：地图页在 `.map-layout` 上方插入 Map timeline；节点 hover/触屏点击浮现“节点操作条”（详情/展开/删除，一体式圆角条浮于圆点上方）；节点按奇偶上下镜像排布：**偶数节点**（第 2/4/…，自左最早→最新计）城市名在上、该段首图缩略图在下，**奇数节点**（第 1/3/…）则缩略图在上、城市名在下；无缩略图的节点只显示城市名（落在圆点对应一侧）。

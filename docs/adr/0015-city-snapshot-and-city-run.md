...
- 前端：地图页在 `.map-layout` 上方插入 Map timeline；节点 hover/触屏点击浮现“节点操作条”（详情/展开/删除，一体式圆角条浮于圆点上方）；节点按奇偶上下镜像排布：**偶数节点**（第 2/4/…，自左最早→最新计）城市名在上、该段首图缩略图在下，**奇数节点**（第 1/3/…）则缩略图在上、城市名在下；无缩略图的节点只显示城市名（落在圆点对应一侧）。展开后的分支行一律是「主体＝在地图上定位该条」+ 行内「详情」（进该次停留的详情页，与侧栏行、信息窗行的行内操作同款 `.row-action`），因此同段内每条停留都有直达详情页的入口。
